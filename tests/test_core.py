...
class TestGetAverageBiomass:
    ...
    def create_mock_statistics(
        self, mean=150.0, std=25.0, count=100, uncertainty_mean=None
    ):
    ...
    def test_basic_biomass_calculation(
        self, mock_validate, mock_stats_proc, mock_footprint_proc, mock_get_source
    ):
        ...
        mock_fp_instance.compute_footprint_weights.assert_called_once_with(
            mock_biomass_data, 52.09, 11.226
        )
        mock_stats_instance.compute_weighted_statistics.assert_called_once()

        # Verify result structure
        assert isinstance(result, pd.DataFrame)
        assert "mean_biomass" in result.columns
        assert "std_biomass" in result.columns
        ...
        # Verify summary data
        assert result.iloc[0]["mean_biomass"] == 150.0
        assert result.iloc[0]["std_biomass"] == 25.0
    ...
    @patch("cosmicbiomass.core.StatisticsProcessor")
    @patch("cosmicbiomass.core.validate_footprint_coverage")
    def test_biomass_output_units_kg_m2(
        self, mock_validate, mock_stats_proc, mock_footprint_proc, mock_get_source
    ):
        ...
        assert "mean_biomass" in result.columns
        assert "mean_biomass_kg_m2" not in result.columns
        assert result.iloc[0]["mean_biomass"] == 10.0
        assert result.iloc[0]["std_biomass"] == 2.0
    ...
    @patch("cosmicbiomass.core.StatisticsProcessor")
    @patch("cosmicbiomass.core.validate_footprint_coverage")
    def test_biomass_with_uncertainty_band(
        self, mock_validate, mock_stats_proc, mock_footprint_proc, mock_get_source
    ):
        ...
        assert result.iloc[0]["uncertainty"] == 15.0
        assert result.iloc[0]["uncertainty_source"] == "uncertainty_band"
        assert result.attrs["result"]["summary"]["uncertainty"] == 15.0
        assert (
            result.attrs["result"]["summary"]["uncertainty_source"]
            == "uncertainty_band"
        )
    ...
    @patch("cosmicbiomass.core.StatisticsProcessor")
    @patch("cosmicbiomass.core.validate_footprint_coverage")
    def test_biomass_without_uncertainty_band(
        self, mock_validate, mock_stats_proc, mock_footprint_proc, mock_get_source
    ):
        ...
        assert result.iloc[0]["uncertainty"] == 25.0  # Same as std
        assert result.iloc[0]["uncertainty_source"] == "data_spread"
        assert result.attrs["result"]["summary"]["uncertainty"] == 25.0
    ...
    def test_footprint_computation_error(self, mock_footprint_proc, mock_get_source):
        ...
        mock_fp_instance = Mock()
        mock_fp_instance.compute_footprint_weights.side_effect = Exception(
    ...
    @patch("cosmicbiomass.core.FootprintProcessor")
    @patch("cosmicbiomass.core.StatisticsProcessor")
    def test_statistics_computation_error(
        self, mock_stats_proc, mock_footprint_proc, mock_get_source
    ):
        ...
        mock_stats_instance = Mock()
        mock_stats_instance.compute_weighted_statistics.side_effect = Exception(
...
class TestGetAverageBiomassTimeseries:
    ...
    def test_timeseries_dataframe_structure(self, mock_get_average):
        ...
        assert "mean_biomass" in df.columns
        assert "std_biomass" in df.columns
    ...
    def test_timeseries_output_units_kg_m2(self, mock_get_average):
        ...
        assert "mean_biomass" in df.columns
        assert "mean_biomass_kg_m2" not in df.columns
        assert df.iloc[0]["mean_biomass"] == 10.0
...
class TestListAvailableDatasets:
    ...
    def test_list_datasets(self, mock_get_source):
        ...
        mock_data_source = Mock()
        mock_datasets = {"agbd_2021": Mock(), "agbd_2020": Mock()}
